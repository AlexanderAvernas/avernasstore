  ...
  try {
    ...
    while (attempts < 5) {
      try {
        ...
        attempts++;
        console.log(`Retry ${attempts} för order ${orderId}:`, error.message);

        if (attempts === 5) {
          console.error("Order hittades inte efter 5 försök:", orderId);
          return new Response(
            JSON.stringify({
              message: "Push received but order not found after retries",
              order_id: orderId,
        ...
        // Progressiv väntetid: 3s, 6s, 9s, 12s
        await new Promise((res) => setTimeout(res, attempts * 3000));
      }
    }

    // Extra säkerhet – orderDetails ska aldrig vara undefined här
    if (!orderDetails) {
      console.error("orderDetails saknas för order:", orderId);
      return new Response(
        JSON.stringify({ message: "No order details", order_id: orderId }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }

    // ✅ Kolla status – skicka bara mejl om ordern är genomförd
    if (orderDetails.status !== "checkout_complete") {
      console.log("Order inte komplett, skippar mejl:", orderId, orderDetails.status);
      return new Response(
        JSON.stringify({ message: "Order not complete", order_id: orderId }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if (!customer?.email) {
      console.error("Ingen kundmail hittades för order:", orderId);
      return new Response(
        JSON.stringify({ message: "No customer email", order_id: orderId }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    // ✅ await mejlet direkt – ingen async IIFE
    await resend.emails.send({
      from: "Margareta Avernas <info@margaretaavernas.se>",
      to: customer.email,
      bcc: "info@margaretaavernas.se",
    ...
    // ✅ Returnerar först efter att mejlet skickats
    return new Response(
      JSON.stringify({
        message: "Push notification received successfully",
        order_id: orderId,
...
export async function POST(req) {
  try {
    ...
    // ✅ Kolla om mejl redan skickats för denna order (idempotency)
    const store = getStore("sent-emails");
    const alreadySent = await store.get(orderId);

    if (alreadySent) {
      console.log("Mejl redan skickat för order:", orderId);
      return new Response(
        JSON.stringify({ message: "Email already sent", order_id: orderId }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    // ✅ Skicka bara mejl om ordern är genomförd
    if (orderDetails.status !== "checkout_complete") {
      console.log("Order inte komplett, skippar mejl:", orderId, orderDetails.status);
      return new Response(
        JSON.stringify({ message: "Order not complete", order_id: orderId }),
    ...
    // ✅ Markera ordern som hanterad INNAN mejlet skickas
    // Detta förhindrar att parallella anrop skickar dubbla mejl
